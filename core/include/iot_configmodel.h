...
struct iot_modelsignal : public iot_releasable { //represents signal from specific node's output. MUST BE ALLOCATED AS MEMBLOCK
	...
	const iot_datavalue* data=NULL; //value or message. CAN BE NULL FOR AND ONLY FOR VALUE LINES
	iot_id_t node_id;
	uint32_t module_id;
...
//				//if false then says that event is NOT response to sync execution of reason_event (reason_event then can be some old event which became the reason).
	...
	iot_modelsignal(iot_nodemodel *model, const char* out_labeln, bool is_msg, uint64_t reltime, const iot_datavalue* msgval=NULL, /*bool is_sync=false,*/ const iot_event_id_t* reason=NULL);
...
struct iot_notify_inputsupdate : public iot_releasable {
	...
	struct {
		int16_t real_index;
		bool is_msg; //real_index is index of msg link or not
		const iot_datavalue* data; //value or message. CAN BE NULL FOR AND ONLY FOR VALUE LINES
	} item[];
...
struct iot_nodemodel {
	...
	struct {
		...
		iot_datavalue_nodeerrorstate* instance_value;
	} erroutput;

	struct {
		iot_config_node_in_t* link;
		const iot_datavalue* instance_value; //last value, received by module instance. i.e. it is current input value from modinstance point of view
	...
	struct {
		...
		const iot_datavalue* instance_value; //last value, set by module instance. i.e. it is current outputted value from modinstance point of view. UPDATED FROM instance thread!!!
...
public:
	...
	int do_update_outputs(const iot_event_id_t *reason_eventid, uint8_t num_values, const uint8_t *valueout_indexes, const iot_datavalue** values, uint8_t num_msgs, const uint8_t *msgout_indexes, const iot_datavalue** msgs);
	const iot_datavalue* get_outputvalue(uint8_t index);
